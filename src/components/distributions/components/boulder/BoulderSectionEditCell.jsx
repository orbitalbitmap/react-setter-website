...
const BoulderSectionEditCell = ({row, field, id, value, valueOptions, formattedValue}) => {
  ...
  return (
    ...
    >
      {
        valueOptions?.map((sectionInfo) => {
          return (
            <MenuItem key={sectionInfo.id} value={sectionInfo.id}>{sectionInfo.name}</MenuItem>
          )
        })
      }
    </Select>
...
export default BoulderSectionEditCell;